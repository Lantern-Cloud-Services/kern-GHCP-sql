...
-- Description: Update an existing employee in tblEmployee and their
...
BEGIN
    ...
    BEGIN
        ...
        IF (@LoginName <> '')
        IF EXISTS(SELECT EmployeeID FROM tblEmployee WHERE IsActive = 'True' AND LoginName = @LoginName AND EmployeeID <> @EmployeeID)
        BEGIN
